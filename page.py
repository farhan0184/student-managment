...
class Student:
	def __init__(self,root):
		...
		def add_data():
			if self.roll_no.get()=="" or self.name.get()==""  or self.father_name.get()=="" or self.contact.get()=="" or self.add_txt.get('1.0',END)=="" or len(self.contact.get())!=11:
				...
				cur=con.cursor()
				cur.execute('insert into stu1 values(%s,%s,%s,%s,%s,%s,%s,%s,%s)',(self.roll_no.get(),self.name.get(),self.father_name.get(),self.gen.get(),self.category.get(),self.branch.get(),self.year.get(),self.contact.get(),self.add_txt.get('1.0',END)))
				con.commit()
				con.close()	
				fetch_data()
				clear_data()
		...
		def fetch_data(): #show into the deshboard from
			con=pymysql.connect(host='localhost',user='root',password='',database='student_management_system')
			cur=con.cursor()
		...
		def update_data():
			if self.roll_no.get()=="" or self.name.get()==""  or self.father_name.get()=="" or self.contact.get()=="" or self.add_txt.get('1.0',END)=="" or len(self.contact.get())!=10:
				...
				con=pymysql.connect(host='localhost',user='root',password='',database='student_management_system')
				cur=con.cursor()
				cur.execute('update stu1 set name=%s , father_Name=%s , gender=%s , category=%s , branch=%s , year=%s , contact_no=%s , address=%s where Roll_no=%s ',(self.name.get(),self.father_name.get(),self.gen.get(),self.category.get(),self.branch.get(),self.year.get(),self.contact.get(),self.add_txt.get('1.0',END),self.roll_no.get()))
				con.commit()
				con.close()
				fetch_data()
				clear_data()
		...
		def delete_data():
			con=pymysql.connect(host='localhost',user='root',password='',database='student_management_system')
			cur=con.cursor()
			cur.execute('delete from stu1 where Roll_no=%s ',self.roll_no.get())
			con.commit()
			con.close()
			fetch_data()
			clear_data()
		...
		def search():
			con=pymysql.connect(host='localhost',user='root',password='',database='student_management_system')
			cur=con.cursor()
			cur.execute("select * from stu1 where "+str(self.search_by.get())+" LIKE '%"+str(self.search_txt.get())+"%'")
			rows=cur.fetchall()
		...
		btn_frame.place(x=15,y=550,width=310,height=120)
		...
		view_frame.place(x=20,y=100,width=930,height=620)
